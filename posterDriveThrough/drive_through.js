...
class ImageDriveThrough {
    ...
    get startIndex(){
        /* The start index of the drive through according to data target */
    ...
    getStartIndex(){
        ...
        Returns int if it was a string and string if it was 'random' */
        let start_idx = this.drive_through.getAttribute('start-index');
        if (start_idx === undefined || start_idx === null || start_idx===""){
            console.log("poster-drive-through class missing custom attribute 'start-index' for initial image load." +
                "Setting as 0");
    ...
    get animateOnLoad(){
        ...
        let bool = this.drive_through.getAttribute('animate-on-load');
    ...
    getDataTargetOrError(){
        ...
        * an error if the attribute is missing or the value doesn't return any elements. */
        ...
        if (check_elem === null || data_target === undefined || check_elem.length===0){
            throw "Invalid data-target on poster-drive-through. There was no element with selected data target found. " +
            "Make sure the data target is a class"
        }
    ...
    getPosterOrError(position){
        ...
        * 'hidden-left' and 'hidden-right' */
        ...
        if (target_element === null || target_element===undefined || target_element.length===0){
            throw "drive-through-poster is missing the html element with class name" + target_classname;
        }
    ...
    loadDriveThrough(){
        /* Loads drive through according to the start-index.
    ...
    get leftPosterImageElement(){
        /* The dom element that contains the left poster */
    ...
    animatePageLoadSlideIn(){
        ...
        if (!this.drive_through.hasAttribute('animate-on-load')){
            console.log("poster-drive-through is missing a custom attribute 'animate-on-load'. Setting as true.")
            this.drive_through.setAttribute('animate-on-load', 'true');
        }
        // If attribute does exist
        else {
            let val = this.drive_through.getAttribute('animate-on-load');
            // if false
    ...
    getHeightSetting(){
        ...
        if (set_fixed===undefined || set_fixed===null){
            console.log("poster-drive-through custom attribute 'fixed-max-height' is missing. Setting as true.")
            this.drive_through.setAttribute('fixed-max-height', 'true');
            return true;
        }
